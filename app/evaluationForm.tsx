  ...
  const [rating,setRating] = useState({
    ...
    const handleRateInstructor = async () => {
      ...
      try {
        ...
        const response = await fetch('https://red-cross-api-final.onrender.com/api/ratings', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
...
async function getInstructorRatings() {
  try {
    ...
    const response = await fetch(`https://red-cross-api-final.onrender.com/api/ratings?userId=${userId}&instructorId=${id}`);
    ...
    <View >
      ...
      {isRated ? 
          ...
          <View style={{flex:1,alignItems:"center",justifyContent:"center",padding:30}}>
            <Text style={{fontSize:30,fontWeight:"300"}}>ALREADY RATED 🎉 </Text>
            ...
            <View style={{gap:20,width:"100%",marginTop:30}} >
              ...
              <View style={{width:"100%",gap:5}}> 
                <Text style={{fontSize:18}}>How would you rate the trainer's ability to explain the course material?</Text>
                <ProgressBar style={{height:10,borderRadius:2}} progress={getProgressValue(rateData?.rate1)} color={ratingLabel(rateData?.rate1)} />
              </View>

              <View style={{width:"100%",gap:5}}> 
                <Text style={{fontSize:18}}>How engaging and interactive was the learning environment created by the trainer?</Text>
                <ProgressBar style={{height:10,borderRadius:2}} progress={getProgressValue(rateData?.rate2)} color={ratingLabel(rateData?.rate2)} />
              </View>


              <View style={{width:"100%",gap:5}}> 
                <Text style={{fontSize:18}}>How effectively did the trainer handle questions and provide feedback during the training session?</Text>
                <ProgressBar style={{height:10,borderRadius:2}} progress={getProgressValue(rateData?.rate3)} color={ratingLabel(rateData?.rate3)} />
              </View>


              <View style={{width:"100%",gap:5}}> 
                <Text style={{fontSize:18}}>How knowledgeable and confident was the trainer in delivering the subject matter?</Text>
                <ProgressBar style={{height:10,borderRadius:2}} progress={getProgressValue(rateData?.rate4)} color={ratingLabel(rateData?.rate4)} />
              </View>


              <View style={{width:"100%",gap:5}}> 
                <Text style={{fontSize:18}}>How well did the trainer adhere to the course objectives and training schedule?</Text>
                <ProgressBar style={{height:10,borderRadius:2}} progress={getProgressValue(rateData?.rate5)} color={ratingLabel(rateData?.rate5)} />
              </View>
        ...
        <>
          ...
          <View>
            <Text style={{fontSize:18,textAlign:"justify"}}>How would you rate the trainer's ability to explain the course material?</Text>
        ...
        <View style={{padding:20}}>
          <View>
            <Text style={{fontSize:18,textAlign:"justify"}}>How engaging and interactive was the learning environment created by the trainer?</Text>
        ...
        <View style={{padding:20}}>
          <View>
            <Text style={{fontSize:18,textAlign:"justify"}}>How effectively did the trainer handle questions and provide feedback during the training session?</Text>
        ...
        <View style={{padding:20}}>
          <View>
            <Text style={{fontSize:18,textAlign:"justify"}}>How knowledgeable and confident was the trainer in delivering the subject matter?</Text>
        ...
        <View style={{padding:20}}>
          <View>
            <Text style={{fontSize:18,textAlign:"justify"}}>How well did the trainer adhere to the course objectives and training schedule?</Text>
        ...
        <TouchableOpacity onPress={ () => {
          ...
          setLoading(true)
                ...
                <Text style={{fontSize:22,fontWeight:"bold",color:"white"}}>{loading ? "LOADING" : "RATE INSTRUCTOR"}</Text>
          </TouchableOpacity>
